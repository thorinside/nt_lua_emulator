-- midi_input_dialog.lua
-- Dialog for selecting MIDI input devices
local M = {}
...
function M.show()
    ...
    if midiHandler.isAvailable() then
        midiPorts = midiHandler.getInputPorts()
        -- Add "None" option at the beginning
        table.insert(midiPorts, 1, {
            index = -1,
            name = "None (Disable MIDI)"
        })
    else
        -- No MIDI support
        midiPorts = {{
...
function M.draw()
    ...
    local title = "Select MIDI Input Device"
    ...
    for i = 1, #midiPorts do
        ...
        if itemY >= listY - itemHeight and itemY < listY + listHeight then
            ...
            if midiHandler.getCurrentPortIndex() == port.index then
    ...
    elseif key == "return" or key == "kpenter" then
        ...
        if selected and midiHandler.isAvailable() then
            if selected.index >= 0 then
                midiHandler.openInputPort(selected.index)
            else
