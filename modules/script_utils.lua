...
function script_utils.trackScriptStepMemory(scriptObj, dt, inputs)
    ...
    scriptMemoryTracking.stepMemory.totalCalls = scriptMemoryTracking.stepMemory.totalCalls + 1
    scriptMemoryTracking.stepMemory.totalMemoryBefore = scriptMemoryTracking.stepMemory.totalMemoryBefore + memoryBefore
    ...
    scriptMemoryTracking.stepMemory.totalMemoryAfter = scriptMemoryTracking.stepMemory.totalMemoryAfter + memoryAfter
    
    local memoryImpact = memoryAfter - memoryBefore
    scriptMemoryTracking.stepMemory.totalImpact = scriptMemoryTracking.stepMemory.totalImpact + memoryImpact
    ...
    if memoryImpact > 0 then
        scriptMemoryTracking.stepMemory.allocations = scriptMemoryTracking.stepMemory.allocations + 1
    end
    
    -- If memory decreased or stayed same, might be due to GC
    if memoryAfter < memoryBefore then
        scriptMemoryTracking.stepMemory.gcTriggered = scriptMemoryTracking.stepMemory.gcTriggered + 1
    end
    ...
    if memoryImpact > 1 then
        debug_utils.debugLog(string.format("Script step(): Memory impact %.2f KB", memoryImpact))
    end
...
function script_utils.trackScriptDrawMemory(scriptObj)
    if not scriptMemoryTracking.enabled then
        return scriptObj.draw(scriptObj)
    end
    ...
    scriptMemoryTracking.drawMemory.totalCalls = scriptMemoryTracking.drawMemory.totalCalls + 1
    scriptMemoryTracking.drawMemory.totalMemoryBefore = scriptMemoryTracking.drawMemory.totalMemoryBefore + memoryBefore
    ...
    scriptMemoryTracking.drawMemory.totalMemoryAfter = scriptMemoryTracking.drawMemory.totalMemoryAfter + memoryAfter
    
    local memoryImpact = memoryAfter - memoryBefore
    scriptMemoryTracking.drawMemory.totalImpact = scriptMemoryTracking.drawMemory.totalImpact + memoryImpact
    scriptMemoryTracking.drawMemory.lastImpact = memoryImpact
    
    -- Track allocations - we estimate this based on positive memory impact
    if memoryImpact > 0 then
        scriptMemoryTracking.drawMemory.allocations = scriptMemoryTracking.drawMemory.allocations + 1
    end
    
    -- If memory decreased or stayed same, might be due to GC
    if memoryAfter < memoryBefore then
        scriptMemoryTracking.drawMemory.gcTriggered = scriptMemoryTracking.drawMemory.gcTriggered + 1
    end
    ...
    if memoryImpact > 1 then
        debug_utils.debugLog(string.format("Script draw(): Memory impact %.2f KB", memoryImpact))
    end
end

-- Track memory usage for script's gate function when available
function script_utils.trackScriptGateMemory(scriptObj, ...)
    if not scriptMemoryTracking.enabled or not scriptObj.gate then
        return nil
    end
    ...
    scriptMemoryTracking.gateMemory.totalCalls = scriptMemoryTracking.gateMemory.totalCalls + 1
    scriptMemoryTracking.gateMemory.totalMemoryBefore = scriptMemoryTracking.gateMemory.totalMemoryBefore + memoryBefore
    
    -- Call the script's gate function
    local result = scriptObj.gate(scriptObj, ...)
    
    local statsAfter = getMemoryStats()
    local memoryAfter = statsAfter.memoryUsage
    local gcCountAfter = statsAfter.gcCount
    
    scriptMemoryTracking.gateMemory.totalMemoryAfter = scriptMemoryTracking.gateMemory.totalMemoryAfter + memoryAfter
    
    local memoryImpact = memoryAfter - memoryBefore
    scriptMemoryTracking.gateMemory.totalImpact = scriptMemoryTracking.gateMemory.totalImpact + memoryImpact
    scriptMemoryTracking.gateMemory.lastImpact = memoryImpact
    
    -- Track allocations - we estimate this based on positive memory impact
    if memoryImpact > 0 then
        scriptMemoryTracking.gateMemory.allocations = scriptMemoryTracking.gateMemory.allocations + 1
    end
    
    -- If memory decreased or stayed same, might be due to GC
    if memoryAfter < memoryBefore then
        scriptMemoryTracking.gateMemory.gcTriggered = scriptMemoryTracking.gateMemory.gcTriggered + 1
    end
    ...
    if memoryImpact > 1 then
        debug_utils.debugLog(string.format("Script gate(): Memory impact %.2f KB", memoryImpact))
    end
...
function script_utils.trackScriptTriggerMemory(scriptObj, ...)
    ...
    scriptMemoryTracking.triggerMemory.totalCalls = scriptMemoryTracking.triggerMemory.totalCalls + 1
    scriptMemoryTracking.triggerMemory.totalMemoryBefore = scriptMemoryTracking.triggerMemory.totalMemoryBefore + memoryBefore
    
    -- Call the script's trigger function
    local result = scriptObj.trigger(scriptObj, ...)
    
    local statsAfter = getMemoryStats()
    local memoryAfter = statsAfter.memoryUsage
    local gcCountAfter = statsAfter.gcCount
    
    scriptMemoryTracking.triggerMemory.totalMemoryAfter = scriptMemoryTracking.triggerMemory.totalMemoryAfter + memoryAfter
    
    local memoryImpact = memoryAfter - memoryBefore
    scriptMemoryTracking.triggerMemory.totalImpact = scriptMemoryTracking.triggerMemory.totalImpact + memoryImpact
    scriptMemoryTracking.triggerMemory.lastImpact = memoryImpact
    
    -- Track allocations - we estimate this based on positive memory impact
    if memoryImpact > 0 then
        scriptMemoryTracking.triggerMemory.allocations = scriptMemoryTracking.triggerMemory.allocations + 1
    end
    
    -- If memory decreased or stayed same, might be due to GC
    if memoryAfter < memoryBefore then
        scriptMemoryTracking.triggerMemory.gcTriggered = scriptMemoryTracking.triggerMemory.gcTriggered + 1
    end
    ...
    if memoryImpact > 1 then
        debug_utils.debugLog(string.format("Script trigger(): Memory impact %.2f KB", memoryImpact))
    end
...
function script_utils.getScriptMemoryReport()
    ...
    if report.trigger.calls > 0 then
        report.trigger.avgImpact = report.trigger.totalImpact / report.trigger.calls
    end
...
function script_utils.printScriptMemoryReport()
    ...
    print(string.format("step():    %d calls, %.2f KB total (%.2f KB avg, %.2f KB peak)",
        report.step.calls, report.step.totalImpact, report.step.avgImpact, report.step.peakImpact))
    print(string.format("          Allocations: %d, GC triggered: %d",
        report.step.allocations, report.step.gcTriggered))
    
    print(string.format("draw():    %d calls, %.2f KB total (%.2f KB avg, %.2f KB peak)",
        report.draw.calls, report.draw.totalImpact, report.draw.avgImpact, report.draw.peakImpact))
    print(string.format("          Allocations: %d, GC triggered: %d",
        report.draw.allocations, report.draw.gcTriggered))
    
    if report.gate.calls > 0 then
        print(string.format("gate():    %d calls, %.2f KB total (%.2f KB avg, %.2f KB peak)",
            report.gate.calls, report.gate.totalImpact, report.gate.avgImpact, report.gate.peakImpact))
        print(string.format("          Allocations: %d, GC triggered: %d",
            report.gate.allocations, report.gate.gcTriggered))
    end
    
    if report.trigger.calls > 0 then
        print(string.format("trigger(): %d calls, %.2f KB total (%.2f KB avg, %.2f KB peak)",
            report.trigger.calls, report.trigger.totalImpact, report.trigger.avgImpact, report.trigger.peakImpact))
        print(string.format("          Allocations: %d, GC triggered: %d",
            report.trigger.allocations, report.trigger.gcTriggered))
    end
    ...
    local totalCalls = report.step.calls + report.draw.calls + 
                      report.gate.calls + report.trigger.calls
    local totalAllocations = report.step.allocations + report.draw.allocations +
                            report.gate.allocations + report.trigger.allocations
